...
const integrationApprovalDetails: Readonly<Record<string, string>> = {
...
export function classifyIntegrationApprovalDiagnostic(
  ...
  stage: ApprovalStage = 'release',
  domain?: string,
): Diagnostic {
  if (diagnostic.detail !== undefined || stage !== 'release' || domain !== undefined) {
    return preserveEvidenceDiagnostics([diagnostic])[0]!;
  }
