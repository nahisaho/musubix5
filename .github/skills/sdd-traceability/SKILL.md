...
After the work, run `npx musubix5 workflow-record sdd-traceability complete
--status completed` exactly once.

1. Run `npx musubix5 trace build`. It scans `.musubix` artifacts and comment
...
2. Run `npx musubix5 trace check --strict --json`.
...
3. Use `npx musubix5 trace impact REQ-FEATURE-001 --json` (or a source path).
...
5. Rebuild after any source/artifact change. Never maintain trace JSON by hand.
   Use `npx musubix5 status` for one-shot readiness; do not start a watcher.
