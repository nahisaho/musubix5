...
Requirements: REQ-M5-COMPAT-001 REQ-M5-COMPAT-002 REQ-M5-COMPAT-003 REQ-M5-COMPAT-005 REQ-M5-COMPAT-006 REQ-M5-COMPAT-007 REQ-M5-COMPAT-013 REQ-M5-LIFECYCLE-005 REQ-M5-RELEASE-001 REQ-M5-RELEASE-003
ADRs: ADR-0002 ADR-0006 ADR-0007 ADR-0010 ADR-0011
Depends-On: DES-M5-003 DES-M5-005 DES-M5-006 DES-M5-008 DES-M5-011 DES-M5-013 DES-M5-014 DES-M5-015 DES-M5-016 DES-M5-017 DES-M5-018 DES-M5-019 DES-M5-020
...
Responsibilities: Validate release event identity, version consistency, candidate/evidence ancestry, current release approval, and operation authorizations; produce a sealed release bundle; and coordinate independent GitHub Release and npm publication jobs.
Interfaces: `.github/workflows/release.yml`, `validateReleaseContext(input)`, `createReleaseBundle(context)`, `verifyReleaseBundle(bundle, expected)`, `releaseOperationOutcome(operation, result)`.
Constraints: Only lightweight release tags are accepted; annotated tags fail before artifact production with `RELEASE_TAG_CANDIDATE_MISMATCH` and an annotated-tag-specific message, making `GITHUB_SHA` the exact candidate commit and strict-OIDC commit claim. A pushed `v*` tag runs validation and bundle production only. Manual dispatch runs only from the same lightweight tag named by `release_tag`, accepts `evidence_commit` only as a full SHA, accepts optional `release_operation_id` and `publish_operation_id` inputs with at least one present, and skips an unrequested side-effect job. It uses a full-history checkout, obtains the default branch from `github.event.repository.default_branch`, verifies that the tagged candidate is an ancestor of `origin/<default>`, and verifies that the evidence commit is a descendant of the tagged candidate and reachable from `origin/<default>`; squash or rebase integration that changes the candidate identity is rejected. Post-candidate evidence is read from a separate checkout by the tagged candidate's built CLI without mutating the tagged candidate workspace. That CLI independently obtains the current release-approval SHA-256 through DES-M5-006, compares it with each authorization record, returns `verifiedReleaseApprovalSha256` for attestation binding, and rejects self-reported or stale approval hashes with `RELEASE_OPERATION_NOT_AUTHORIZED`. Version validation covers every `version` value in `package.json`, `packages/*/package.json`, `plugin.json`, and both the catalog and plugin entries in `.github/plugin/marketplace.json`. The validation job requires typecheck, build, full tests, compatibility tests, package checks, and package smoke tests to exit zero; formal classification is advisory. Each workflow run produces and verifies its own run-scoped bundle; a tag-push bundle is validation evidence only and is never consumed by a later dispatch. Bundle production and publication both use `ubuntu-24.04` and the design-approved exact npm 11.5.1 pin; changing the pin requires renewed design approval. Bundle production performs two complete clean checkout, install, build, and pack cycles and fails with `RELEASE_PACKAGE_DIGEST_MISMATCH` unless their tarballs are byte-identical, then creates a CycloneDX SBOM and `SHA256SUMS` and signs a GitHub-OIDC-authorized ephemeral Ed25519 attestation. All release-specific expected bindings are canonicalized into equality-checked `evidenceHeads` digests: release context binds mode `tag-push | dispatch`, repository, candidate, workflow, tag, and on dispatch the evidence commit and independently derived release approval; release bundle binds the SHA-256 of `SHA256SUMS`. Release transport uses a closed code-defined strict policy derived from the existing attestation primitives, fixes workflow to `.github/workflows/release.yml`, `maxAgeSeconds` to 2592000, and `maxFutureSkewSeconds` to 60, and does not add a user-configurable schema key or reuse the candidate-gate workflow identity. Side-effect jobs download and independently verify the sealed bundle using strict issuer, audience/key, repository, candidate, workflow, mode, tag, context, and checksum checks. The publish job clean-checks out the tagged candidate, uses the pinned runner and npm version, rebuilds and repacks it, fails with `RELEASE_PACKAGE_DIGEST_MISMATCH` unless the regenerated tarball SHA-256 equals the sealed `SHA256SUMS` entry, and then publishes from the package source directory under the explicit invariant that the pinned `npm publish` packing algorithm equals the verified `npm pack` algorithm. After publication it computes local SRI as `sha512-<base64>` over the verified tarball and fails post-hoc with `RELEASE_REGISTRY_INTEGRITY_MISMATCH` without implying rollback unless the registry `dist.integrity` is identical. The release and publish jobs depend on bundle production but not on each other, use their explicit operation-ID inputs, use operation-specific concurrency with `cancel-in-progress: false`, validate schema-version-2 `status: authorized` records through the candidate-built read-only CLI, reject existing targets, emit run-scoped outcome artifacts, and receive minimum job permissions. The publish job uses the protected `npm-publish` environment; Trusted Publishing omits `--provenance`, while token fallback includes it.
Requirements: REQ-M5-RELEASE-001 REQ-M5-RELEASE-003
ADRs: ADR-0008 ADR-0010 ADR-0011
Depends-On: DES-M5-006 DES-M5-014 DES-M5-015 DES-M5-016 DES-M5-019
...
| release workflow diagnostics | `RELEASE_TAG_CANDIDATE_MISMATCH`, `RELEASE_VERSION_MISMATCH`, `RELEASE_OPERATION_NOT_AUTHORIZED`, `RELEASE_PACKAGE_DIGEST_MISMATCH`, and post-hoc `RELEASE_REGISTRY_INTEGRITY_MISMATCH` | Intentional fail-closed release surface governed by REQ-M5-RELEASE-001, REQ-M5-RELEASE-003, and ADR-0011 |
| release workflow | lightweight-tag validation, explicit operation-ID dispatch inputs, sealed bundle, release-specific strict OIDC policy, and independent release/publish jobs | Intentional release automation governed by REQ-M5-RELEASE-003 and ADR-0011 |
