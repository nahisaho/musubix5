...
Type: functional
Pattern: ubiquitous
Statement: The system shall register the lifecycle, approval, workflow, candidate-bound release, candidate-snapshot CLI, parallel-development, multi-CHANGE workspace, shared trace index, and scoped TDD repair additions defined by REQ-M5-LIFECYCLE-005, REQ-M5-APPROVAL-007, REQ-M5-EVIDENCE-006, REQ-M5-EVIDENCE-007, REQ-M5-RELEASE-002, REQ-M5-TDD-003, REQ-M5-WORKTREE-001, REQ-M5-WORKTREE-005, REQ-M5-WORKTREE-006, REQ-M5-WORKTREE-007, REQ-M5-PARALLEL-013, REQ-M5-PARALLEL-017, REQ-M5-MULTI-CHANGE-001 through REQ-M5-MULTI-CHANGE-008, REQ-M5-WAVE1-TRACE-001 through REQ-M5-WAVE1-TRACE-002, and REQ-M5-WAVE1-TDD-001 through REQ-M5-WAVE1-TDD-002 as intentional extensions to the pinned musubix3 contract.
Acceptance: ADRs, migration-guide entries, additive help and JSON-field registries, and regression tests cover configuration projections, versioned schemas, candidate-tree sourcing, the complete REQ-M5-WORKTREE-005 through REQ-M5-WORKTREE-007 public candidate-snapshot lifecycle and its diagnostics, full lowercase hexadecimal Git object IDs from 40 through 64 characters at every changed persistence, validation, workflow, and shell boundary, exclusion predicates and reasons including current `.musubix/trace/index.json` and migration-era `.musubix/features/*/trace.json` as `generated-trace`, matching semantics, sanitization modes, candidate-bound gate evidence, parallel worktree, provenance-gated TDD, integration evidence, deterministic cross-platform candidate/integration filesystem-key encoding without legacy literal-ID fallback, abandoned-generation maintenance records and their non-credit schema, audited workflow declaration correction records and their non-credit projection, the additive shared trace index schema/load precedence/legacy read-only fallback and evidence-input exclusions, the additive `tdd repair [test-id]` normal, `--resume`, and `--abandon-pending` help contracts, domainless authorization JSON `domain: null`, pending-repair machine-readable detail fields `operationId`, `testId`, and `targetCycleId`, deterministic `TDD_REPAIR_TARGET_INVALID` cause values, non-credit `repairAbandonments[]`, `TDD_REPAIR_TARGET_INVALID`, `TDD_REPAIR_BINDING_MISMATCH`, `TDD_REPAIR_ALREADY_RECORDED`, `TDD_REPAIR_REPLACEMENT_INVALID`, `TDD_REPAIR_CHAIN_INVALID`, `TDD_REPAIR_AUTHORIZATION_INVALID`, `TDD_REPAIR_PENDING`, `TDD_REPAIR_RESUME_INVALID`, and `TDD_REPAIR_ABANDON_INVALID` as exit-1 domain diagnostics, the `TDD_REPAIR_*` CLI exit-code registry extension, every other added or changed diagnostic and displayed output, and changed aggregate hashes before implementation; unregistered differences remain compatibility failures.
...
Multi-Change-Selection-Correction: Normative replacement for REQ-M5-MULTI-CHANGE-001 through REQ-M5-MULTI-CHANGE-008: replace “requires at most one active or legacy-active CHANGE document” with “permits multiple active or legacy-active CHANGE documents, each with at most one positive active generation”. A legacy-active document counts toward implicit selection ambiguity. Implicit resolution still requires exactly one selectable CHANGE and retains the existing no-active and `CHANGE_GENERATION_MIXED` exit, status, and no-persistence behavior. An explicit `--change-id` or registered candidate-workspace context selects exactly one matching active CHANGE and positive generation without making another active CHANGE eligible or current; all maintenance exceptions and abandoned-generation restrictions remain unchanged.
...
## REQ-M5-TDD-003: Evaluate TDD cycles deterministically
Priority: must
Type: functional
Pattern: state-driven
Statement: While TDD evidence is evaluated, the system shall determine requirement coverage and source-currency status from verified monotonic evidence order.
Acceptance: A complete coverage cycle has strict Red order before Implementation order before Green order and binds one CHANGE generation, requirement, test, command, and candidate lineage; a parallel assignment cycle additionally requires current provisional or verified integration provenance that consumed its exact assignment attempt and commit range. Coverage credit is restricted to eligible cycles in the active CHANGE scope and generation, unconsumed parallel cycles are reported with `PARALLEL_TDD_UNCONSUMED`, prior-generation cycles are reported as superseded and cannot satisfy coverage, and repeated evaluation yields the same coverage result while explaining excluded records. Source-currency selection is distinct from requirement coverage and considers all CHANGE scopes and generations solely to evaluate current test-source staleness; foreign evidence grants no requirement-coverage, provenance, or parallel-consumption credit and affects only source-currency evaluation. The active scope predicate is `(cycle.generation ?? 1) === activeGeneration` and an absent `changeId` key or `cycle.changeId === activeChangeId`; the stale-check membership predicate additionally requires valid Green evidence, meaning a linked and verified Green phase whose observed test result passed. While a CHANGE is active, source-currency validation evaluates test IDs satisfying stale-check membership, using persisted ownership fields at evaluation time including ownership adopted by normal phase recording. When no CHANGE is active, source-currency validation evaluates every test ID having at least one persisted cycle with valid Green evidence. If a selected CHANGE has no active generation, validation retains the existing `CHANGE_GENERATION_PHASE` exit-2 failure, while `tdd migrate` and `tdd void` preserve their maintenance behavior by treating the state as no active operation scope and considering all cycles for the test. For each evaluated test ID, an eligible source-currency cycle has valid Red and Green phases whose verified Red order is strictly less than their verified Green order; an Implementation phase is not required for source-currency eligibility. Each credited Red, Green, Refactor, migration, or void phase has exactly one chain record matching `(phase, cycleId, testId)` in the complete chain, with that unique record's position determining the expected predecessor digest; Red and Green phases have verified cycle identity, test identity, payload digest, hash-chain predecessor, and monotonic order linkage; and the cycle is not validly voided. Its terminal fingerprint order is the greatest verified order among (a) its Green phase, (b) a valid Refactor phase whose payload and chain/order linkage are verified, and (c) a valid fingerprint migration whose payload and chain/order linkage are verified and whose trimmed human approver is non-empty. The indexed greatest verified event for validation is selected by monotonic order from eligible terminal fingerprints, valid voids, and linked work events: valid Red on a cycle without valid Green, or a linked Green phase whose result is not valid passing Green. With an active CHANGE, only work events satisfying the active scope predicate can suppress. With no active CHANGE, work events whose `changeId` is absent can suppress any selected terminal, while scoped work events can suppress only when their `(changeId, effective generation)` equals the unbounded greatest eligible terminal selection's scope; when no eligible terminal exists validation emits no staleness diagnostic. This unbounded terminal scope is fixed before any valid-void bound is applied. Foreign or prior-generation scoped open work cannot suppress an active target's source-currency diagnostic. When the greatest validation event is open or non-passing qualifying work, source-currency validation emits no `TDD_TEST_STALE` until that cycle obtains valid Green or is validly voided. When the greatest event is a valid void, the effective cycle is the eligible cycle with the greatest terminal fingerprint order strictly less than that void order; a cycle whose terminal Refactor or migration is at or above that bound is excluded rather than falling back to its earlier Green. Otherwise the effective cycle is the eligible cycle with the greatest terminal fingerprint order. Validation, fingerprint migration, and void fallback obtain verified event order and any exclusive bound from the same index rather than evidence-array position; each consumer applies its operation-specific event scope. Fingerprint migration requires a trimmed non-empty approver before persistence and retains the existing thrown `An approver is required to migrate TDD fingerprint evidence.` error with exit code 2 for a blank or whitespace-only value; while a CHANGE is active, its target terminal, void, and work candidates are restricted to the active scope predicate, while without an active CHANGE it applies validation's no-active qualification rules. If no cycle for the test exists in any scope, migration retains `No TDD cycle found for <testId>.` as an exit-2 error; if cycles exist globally but no eligible terminal exists in the active operation scope, it retains `<testId> has no valid Green phase to migrate.` as an exit-2 error without persistence. A cycle-ID-less or chain-unverified record cannot become the selected eligible migration terminal and therefore uses the no-valid-Green error instead of the legacy `<testId> lacks a cycle ID; regenerate its evidence before migrating.` guard. After verified selection, any persisted migration record—valid or malformed—retains `<testId> has already been migrated.` as an exit-2 error and is never overwritten or duplicated; malformed migration evidence remains diagnosed and requires a genuine new Red/Green cycle to establish later current evidence. When its greatest qualifying event is open or non-passing work it preserves the same no-valid-Green rejection without persistence; otherwise it uses the selected cycle and fingerprint and applies a greatest-event void order as the exclusive bound. An active-scope migration can be valid for maintenance yet remain globally non-current when a later valid foreign or prior-generation void bounds repository-wide validation; this is intentional, persists no foreign mutation, and requires completing or abandoning the active CHANGE before repository-wide repair. Void recording retains `No TDD cycle found for <testId>.` as an exit-2 error only when no cycle for the test exists in any scope. It then applies the active scope predicate when a CHANGE is active, otherwise considers all cycles for the test. Before verified-event targeting it retains the existing structural guards on the array-last cycle within that operation scope solely for backward-compatible missing-cycle-ID and missing-chain exit-2 errors; the missing-chain guard's companion “another cycle exists” test uses only cycles in the same operation scope. Those structural guards take precedence over valid-Green and already-voided message selection, and array position grants no target or fallback priority. It then computes the order-greatest verified event among terminal fingerprint evidence (Green, valid Refactor, or valid approved migration), valid voids, and work events for the candidate scope. A terminal-fingerprint greatest event belongs to a cycle with valid Green and preserves the existing valid-Green rejection regardless of whether Green, Refactor, or migration supplied that terminal order; a valid void greatest event preserves the existing already-voided rejection; and a greatest work event of either kind—valid Red on a cycle without valid Green, or linked non-passing Green—becomes the target when its cycle has no persisted void record and no valid Green. A cycle bearing an unverified void record is not eligible for re-targeting or overwrite and instead returns `{ voided: false, reason: "<testId> has no verifiable dangling TDD cycle in the current operation scope." }` with exit code 1 without persistence. Valid void records require exactly one chain record matching `(void, cycleId, testId)` in the complete chain, with that unique record's position determining the expected predecessor digest; duplicate void-chain records are unverified and cannot bound selection or become the greatest verified event. The fallback is selected from eligible candidates satisfying the same operation scope predicate used for void targeting, strictly before that work-event order. When cycles exist globally but no linked target exists in the active operation scope, it returns the same `{ voided: false, reason: "<testId> has no verifiable dangling TDD cycle in the current operation scope." }` result with exit code 1 without persistence. The retained valid-Green and already-voided failures remain CLI errors with exit code 2 and their existing message strings, where “latest cycle” means the order-greatest verified cycle event rather than array position; which retained message applies to a repository state can change under verified-event precedence or structural-guard precedence. The no-fallback reason string remains byte-identical to the existing text even though fallback eligibility is determined by verified order and active operation scope. If bounded selection yields no cycle, validation emits no staleness diagnostic, fingerprint migration rejects because no valid Green fallback is selectable, and void recording rejects because it would leave no eligible fallback. A repository-wide invalid or duplicate monotonic order log yields no verified source-currency candidate and retains its non-pass order diagnostics. A migration whose trimmed approver is empty emits the same `TDD_LEGACY_OR_UNSCOPED_EVIDENCE` diagnostic used for a missing approver and cannot become terminal currency evidence. When no eligible cycle or no current trace node exists for an evaluated test ID, source-currency validation emits no staleness diagnostic for that test and retains the applicable missing, malformed, invalid-evidence, or trace diagnostics. Any `TDD_TEST_STALE` diagnostic uses the current trace test node's path rather than a historical cycle path. Evidence-array position, active-CHANGE filtering of candidate fingerprints, superseded coverage status, and legacy unscoped status do not override terminal fingerprint order for source-currency selection, so an older legacy unscoped cycle cannot shadow a later verified scoped cycle for an evaluated test. Array-position supersession classification remains outside this source-currency correction and does not determine the selected currency fingerprint. The changed source-currency work-event suppression and stale-diagnostic applicability, order-based applicability of retained `tdd void` messages, active-scope applicability and verified-selection precedence of retained migration messages, active-scope applicability of the void-fallback message, foreign-only exit-1 void result during an active CHANGE, trimmed migration-approver rejection and diagnostic applicability, and active-scope restrictions are intentional behavioral differences registered under REQ-M5-COMPAT-013 and ADR-0023. `TEST-M5-TDD-EFFECTIVE-LATEST-001` verifies the primary cross-CHANGE legacy/scoped verified-terminal ordering regression. `TEST-M5-TDD-MAINTENANCE-SCOPE-001` verifies the exact unverified-void no-overwrite exit-1 behavior. Unannotated companion cases in `tests/tdd-generation-scope.test.ts` supplement regression coverage for active and no-active membership and work suppression, migration selection and retained messages, void targeting and fallback, duplicate and invalid evidence, an out-of-scope cycle whose verified Green order is not strictly after its verified Red order being ineligible and supplying no terminal fingerprint or `TDD_TEST_STALE`, missing trace nodes, source-text and per-test fingerprint cache isolation, multiple test IDs in one source file, valid-void bounds, Refactor/migration precedence, and compatibility-registry and migration-guide source assertions; those companion cases are not separate trace entities. Provisional provenance can satisfy only TDD coverage during integration verification; release readiness and cleanup still require passing integration verification and verified provenance.
...
Multi-Change-Create-Correction: Normative detail: the preceding sole-active confirmation-token rule remains unchanged in the control root. Inside a registered live candidate workspace, workspace ownership explicitly selects its CHANGE and positive generation before and after lease acquisition; the positional `<change-id>` remains a byte-exact confirmation token, another active CHANGE does not cause `CHANGE_GENERATION_MIXED`, and a token/workspace mismatch fails exit 1 `CANDIDATE_WORKSPACE_SELECTOR_MISMATCH` without persistence.
...
Multi-Change-Projection-Correction: Normative detail: within a valid registered candidate context, “sole active CHANGE” and “sole active generation” eligibility checks in this requirement mean the explicitly selected CHANGE and its sole positive active generation. Other active CHANGE documents do not by themselves produce `not-current`, `stale`, `conflicting`, or non-eligible status; records from those CHANGEs remain visible but grant no credit to the selected candidate.
...
Multi-Change-Stale-Protection-Correction: Normative detail: a candidate-scoped release approval invalidated solely because REQ-M5-MULTI-CHANGE-006 integration advanced the default branch or REQ-M5-MULTI-CHANGE-007 refresh is required has `releaseApprovalStatus: stale` and `protected: false`; audited snapshot deletion is then permitted without reopening the CHANGE generation. The tombstone persists that stale pre-deletion status and invalidates the old candidate gate, release manifest, release approval, and attestation bindings.
...
Multi-Change-Release-Correction: Normative detail: release preparation, recording, gate evaluation, and readiness inside a registered candidate context select only that candidate's CHANGE, positive generation, repository identity, and sole live snapshot. Other active CHANGE documents do not by themselves block or stale the selected candidate evaluation, while omitted selection in a mixed repository retains `CHANGE_GENERATION_MIXED`; no approval, snapshot, gate record, or attestation from another candidate grants credit. A candidate-context release approval is candidate-scoped pre-integration evidence and grants no repository publication credit. Integration or refresh under REQ-M5-MULTI-CHANGE-006 and REQ-M5-MULTI-CHANGE-007 stales every release approval, gate record, and attestation whose bound candidate commit differs from the verified integration commit. Publication readiness under REQ-M5-RELEASE-002 and REQ-M5-RELEASE-003 requires fresh release approval and complete gate evidence bound to the verified integration commit after that commit is reachable from the default branch.
...
- parent `tdd --help` adds the `repair` subcommand entry
- `tdd repair [test-id]` adds the optional positional grammar; normal mode
  requires the positional and adds `--cycle <cycle-id>`, mutually exclusive
  `--replacement-cycle <cycle-id>` / `--retire`, `--approver <name>`,
  `--reason <text>`, `--confirm` default `false`, and common
  `--root <directory>` / `--json`
- `tdd repair --resume <operation-id>` adds recovery mode with `--confirm`
  default `false`, common `--root <directory>` and `--json`, and is mutually
  exclusive with `<test-id>`, `--cycle`, `--replacement-cycle`, `--retire`,
  `--approver`, and `--reason`
- `tdd repair --abandon-pending <operation-id>` adds audited non-credit
  recovery with `--approver <name>`, `--reason <text>`, `--confirm` default
  `false`, common `--root <directory>` / `--json`, and is mutually exclusive
  with `<test-id>`, `--resume`, `--cycle`, `--replacement-cycle`, and `--retire`
- repair JSON adds success fields `repaired`, `replayed`, `resumed`,
  `operationId`, `testId`, `targetCycleId`, `disposition`,
  `replacementCycleId`, `fallbackCycleId`, `approver`, `reason`, and `order`;
  abandonment success adds `abandoned` and closed `failureCause` values
  `journal-invalid`, `identity-mismatch`, `partial-projection-mismatch`; schema registration
  adds `tdd-repair-v1`, `tdd-repair-abandonment-v1`,
  `repairAbandonments[]`, `pendingJournalSha256`, and
  `partialFragments[]` fields `kind`, `index`, `sha256`;
  error details add authorization fields `requirementId`, `domain`, `status`,
  target fields `cause`, `testId`, `targetCycleId`, pending fields
  `operationId`, `testId`, `targetCycleId`, and resume fields `operationId`,
  `cause`; authorization `domain` accepts `null`, and abandonment errors add
  `operationId` and `cause`
- repair compatibility registers authorization statuses `missing`, `stale`,
  `domain-mismatch`; target-invalid causes, in precedence order,
  `cycle-not-found`, `cycle-id-ambiguous`, `foreign-operation-scope`,
  `target-not-parallel-bound`, `target-parallel-binding-not-stale`,
  `retirement-fallback-missing`, `retirement-fallback-ambiguous`; resume causes
  `not-found`, `already-completed`,
  `journal-invalid`, `identity-mismatch`, `partial-projection-mismatch`;
  abandonment causes `not-found`, `already-completed`, `resumable`,
  `journal-envelope-invalid`, `already-abandoned`; their specified precedence;
  exact normal/resume/abandon CLI errors; exact messages and detail shapes for
  `TDD_REPAIR_TARGET_INVALID`, `TDD_REPAIR_BINDING_MISMATCH`,
  `TDD_REPAIR_ALREADY_RECORDED`, `TDD_REPAIR_REPLACEMENT_INVALID`,
  `TDD_REPAIR_CHAIN_INVALID`, `TDD_REPAIR_AUTHORIZATION_INVALID`,
  `TDD_REPAIR_PENDING`, `TDD_REPAIR_RESUME_INVALID`, and
  `TDD_REPAIR_ABANDON_INVALID`; repair and abandonment success text; and all corresponding
  help, JSON, diagnostics, displayed-output, filesystem-effect, and aggregate
  hashes; compatibility fixtures preserve quarantined journal/order/repair/
  chain bytes and order/chain predecessor continuity while excluding them only
  from effective repair projection, coverage/source-currency credit, and future
  cycle selection
...
- `candidate-workspace` adds the public
  `create --change-id <id>`, `list`, `show <change-id-or-candidate-id>`,
  `ready <change-id-or-candidate-id>`, `resume <change-id-or-candidate-id>`,
  `refresh --change-id <id>`,
  `integrate <change-id-or-candidate-id...> --confirm`, and
  `cleanup <selector> --deleted-by <name> --confirm` lifecycle under
  REQ-M5-MULTI-CHANGE-001 through REQ-M5-MULTI-CHANGE-008; the top-level
  `musubix5` and `musubix5 help` output insert its command-group line after
  `candidate-snapshot`; all child commands register common `--root` and
  `--json`; all `approval prepare`, `approval record`, and
  `approval validate` stages change the existing hidden release-only
  `--change-id <id>` into a visible all-stage selector, and generation-bound
  `tdd`, `gate`, and `status` commands add the same visible option without
  adding a new status JSON field; workflow retains its registered
  `--change-id`; trace, graph, gate, and status add hidden
  integration-verification option
  `--multi-change-verification <integration-id>`; registered
  diagnostics are `CANDIDATE_WORKSPACE_NOT_FOUND`,
  `CANDIDATE_WORKSPACE_REPOSITORY_MISMATCH`,
  `CANDIDATE_WORKSPACE_GENERATION_MISMATCH`,
  `CANDIDATE_WORKSPACE_SELECTOR_MISMATCH`, `CANDIDATE_EVIDENCE_MISMATCH`,
  `CANDIDATE_STATE_OWNERSHIP`, `CANDIDATE_LEASE_BUSY`,
  `CANDIDATE_JOURNAL_BUSY`,
  `CANDIDATE_OPERATIONAL_STATE_DRIFT`,
  `CANDIDATE_OWNERSHIP_CONFLICT`, `CANDIDATE_DEPENDENCY_UNSATISFIED`,
  `CANDIDATE_DEPENDENCY_CYCLE`, `CANDIDATE_BASE_STALE`,
  `CANDIDATE_COMMIT_UNREACHABLE`, `CANDIDATE_ALREADY_INTEGRATED`,
  `CANDIDATE_INTEGRATION_CONFLICT`,
  `CANDIDATE_INTEGRATION_VERIFICATION_FAILED`, and
  `CANDIDATE_CLEANUP_UNSAFE`; registered JSON fields are the candidate
  identity, CHANGE/generation/repository ownership, branch/worktree/base/tip
  commits, dependency order, lifecycle state, manifest digest, verification
  report hashes, lease/audit metadata, and recovery guidance defined by those
  requirements; versioned persistence schemas are
  `candidate-workspace-registry-v1`, `candidate-workspace-record-v1`,
  `candidate-integration-v1`, `candidate-workspace-tombstone-v1`, and
  `repository-append-lease-v1`; `integration-source-manifest-v1` is the
  registered deterministic integration projection; integration verification
  registers diagnostic details `integration-candidate-snapshot-missing`,
  `integration-candidate-gate-missing`, `integration-candidate-context-mismatch`,
  `integration-release-approval-missing`, and
  `integration-release-approval-stale`
