...
Interfaces: `startIntegration(planId)`, `resumeIntegration(planId)`, `reopenIntegration(input)`, `verifyIntegration(planId)`, `currentIntegrationProvenance(planId)`, `resolveIntegrationPreReleaseContext(root, plan)`, `integrationGateAcceptable(result, context)`, `integrationStatusAcceptable(result)`.
Constraints: A stopped command resumes one attempt; only reopen increments it. The caller's named completed assignment set must be dependency-closed over completed transitive dependents; an incomplete set fails `PARALLEL_ASSIGNMENT_STATE` without mutation. Reopen transitions only that explicit named set to failed, then recomputes non-named descendant projections and start commits; every other persisted completed transition remains unchanged. There is no automatic content conflict resolution; provisional provenance is plan-wide and invalidated completely on reopen; verification runs every required configured command plus strict trace, graph gate, `gate --changed --parallel-verification <plan-id>`, and status in the integration worktree overlay. `integrationGateAcceptable` accepts completed JSON exit 0 only when every required check passes. For JSON exit 1, `resolveIntegrationPreReleaseContext` reads authoritative CHANGE evidence, candidate snapshot records, and release approval evidence from the control root and returns a context only when the plan-bound CHANGE is the sole active CHANGE, generation-equal to the plan, pre-quality, and has no live repository-matching candidate. Persisted release approval is classified as absent, same-CHANGE, or foreign-completed by exact `changeId` and CHANGE document/evidence state; absent evidence permits only `APPROVAL_CANDIDATE_MISSING`, foreign-completed evidence permits `APPROVAL_CANDIDATE_MISSING` or `CHANGE_GENERATION_INCOMPLETE`, and same-CHANGE or unbound foreign evidence fails closed. Exit-1 acceptance derives failures only from required checks, requires at least one, permits only `change-history` and `approval`, and validates each failure only when present. A failing `change-history` has one error `CHANGE_GENERATION_INCOMPLETE`, one error or authoritative-waiver warning `CHANGE_PHASE_MISSING` with detail `phase:quality`, and no other error; a failing `approval` has one context-permitted error. Either check may pass or be non-required without rejection. Required diagnostics must use a recognized `error|warning|info` severity, and the warning phase-missing member must carry authoritative waiver attribution; malformed required diagnostic shapes fail closed. Every non-required check outcome, including `fail`, `skipped`, or an unrecognized status with arbitrary diagnostics, is retained verbatim but excluded from both exit-0 and exit-1 acceptance decisions and grants no credit. Because separate workspace overlays do not persist the integration gate report for the following process, `integrationStatusAcceptable` treats status as an independent non-readiness assertion and requires initialized JSON exit 0, `gate.status` in `pass|fail|stale|skipped`, and `gate.ready = false`; it never treats status as proof of the preceding gate result. DES-M5-006 `classifyReleaseApprovalDiagnostic` remains available for reporting and recovery guidance only and never decides integration acceptance. The exported `integrationGateAcceptable` signature gains the resolved context as an explicit compatibility addition; `integrationStatusAcceptable` remains module-private. Verified provenance retains raw gate/status outcomes and tolerated diagnostics, gives no terminal quality or release credit, and remains eligible for REQ-M5-PARALLEL-012 handoff and branch-retaining cleanup. Every absent, malformed, failed, skipped, stale or mismatched required gate state, ready status, or context-mismatched outcome fails verification and leaves provenance provisional.
...
ADRs: ADR-0005 ADR-0010 ADR-0012 ADR-0017
